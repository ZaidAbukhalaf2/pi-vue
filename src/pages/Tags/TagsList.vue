<template>
  <div class="pt-3 pb-2 mb-3 border-bottom">
    <router-link class="btn btn-sm btn-outline-secondary" to="/tag/create"
      >Add</router-link
    >
  </div>
...
export default {
  ...
  setup() {
    ...
    const allTags = (page = 1) => {
      axios.get(`admin/tags/all-tags/?page=${page}`).then((res) => {
    ...
    const deleteTag = (id) => {
      if (confirm("Are you sure you want to delete?")) {
        axios.delete(`admin/tags/delete/${id}`).then((res) => {
